...
function filterStudentsByGrade(students, grade) {
  ...
  let arr = students.filter((e) => grade == e['grade'])
  arr.forEach(e => {
    console.log(e.name)
  });
}
...
function compareArrays(arr1, arr2) {
  ...
  arr1.sort((a, b) => a - b)
  arr2.sort((a, b) => a - b)
  let jsArr1 = JSON.stringify(arr1)
  let jsArr2 = JSON.stringify(arr2)
  console.assert(jsArr1 === jsArr2, 'Масиви не однакові')
}
...
function displayGroupedInfo(students) {
  ...
  // Закриємо групу
  for (const iter of students) {
    console.group(`Студент: ${iter.name}`)
    console.info(`Вік: ${iter.age}`)
    console.info(`Предмет: ${iter.subject}`)
    console.groupEnd()
  }
}
...
function calculateTotalPrice(products) {
  ...
  let total = 0
  total = products.reduce((a, e) => a + e['price'], 0)
